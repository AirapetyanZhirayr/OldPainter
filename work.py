...
args = {
    ...
    'grid_div': sorted([5, 6, 7, 8]) if FAST_TESTING else sorted([3, 4, 5]),
    'max_m_strokes': 174*1 if FAST_TESTING else 270,
...
with open(os.path.join(interaction_dir, 'blank.txt'), 'w') as f:
    f.write('delete')
    ...
    for j, pt.m_grid in enumerate(args['grid_div']):
        ...
        for pt.anchor_id in range(0, pt.m_strokes_per_block):
            if args['sub_experiment']:
                ...
                stroke_params = np.c_[
                    pt.x_ctt[:, pt.anchor_id, :].detach().numpy(),
                     pt.x_w[:, pt.anchor_id, :].detach().numpy(),
                     pt.x_h[:, pt.anchor_id, :].detach().numpy(),
                     pt.x_color[:, pt.anchor_id, :].detach().numpy(),
                     pt.x_alpha[:, pt.anchor_id, :].detach().numpy()
                ]
